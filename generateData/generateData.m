...
prb = prb .* sqrt(P.prbFlux/intensity);
...
% Generate C for monochromatization (if it doesn't exist already):
if exist(['/C_' num2str(P.rel_bw*100) '.mat'],'file') == 0
    C = generateC(P);
    save(['C_' num2str(P.rel_bw*100) '.mat'],'C')
else
    load(['C_' num2str(P.rel_bw*100) '.mat'],'C');
end
P.C = C;
